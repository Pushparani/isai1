...
class com.jeroenwijering.players.ImageModel extends AbstractModel { 
	...
	function ImageModel(vws:Array,ctr:AbstractController,car:Object,far:Array,imc:MovieClip,scl:Boolean) {
		...
		if(arguments[5] == true) {
			imageLoader = new ImageLoader(imageClip,configArray["overstretch"],
				configArray["width"],configArray["height"]);
		} else {
		...
		imageLoader.onLoadFinished = function() {
			...
			if(ref.isSWF == true) {
				...
				if(ref.imageLength < ref.configArray["rotatetime"]) { 
					ref.imageLength = ref.configArray["rotatetime"];
				}
	...
	private function setStart(pos:Number) {
		...
		if(fileArray[currentItem]["file"] != currentURL) {
			...
			imageLength = configArray["rotatetime"];
			if(fileArray[currentItem]["file"].indexOf(".swf") == -1) {
				isSWF = false;
			} else {
				isSWF = true;
			}
