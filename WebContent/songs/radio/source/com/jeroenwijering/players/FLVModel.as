...
class com.jeroenwijering.players.FLVModel extends AbstractModel { 
	...
	/** Constructor **/
	function FLVModel(vws:Array,ctr:AbstractController,car:Object,far:Array,
		fcl:MovieClip) {
	...
	private function setItem(idx:Number) {
		...
		if(isActive == true) {
			if(configArray["streamscript"] != "undefined") { 
				flvType = "HTTP";
			} else if(fileArray[currentItem]["file"].indexOf("rtmp://")!=-1) {
				flvType = "RTMP"; 
			} else { 
				flvType = "FLV"; 
			}
	...
	private function setStart(pos:Number) {
		...
		if (flvType == "RTMP" && fileArray[currentItem]["id"] != currentURL) {
			...
			fileArray[currentItem]["file"] != currentURL) {
			...
			if(flvType == "HTTP" ) {
				...
				if(configArray["streamscript"] == "lighttpd") {
					...
					streamObject.play(configArray["streamscript"] + 
					"?file=" + currentURL);
				}
			} else {
				setStreamObject(connectObject);
	...
	private function updatePosition() {
		...
		if(pos == currentPosition && currentState != 1 && 
			streamObject.bufferLength < configArray["bufferlength"]-1) {
			currentState = 1;
			sendUpdate("state",1);
		...
		for (var i=0; i< metaKeyframes.times.length; i++) {
			if((metaKeyframes.times[i] <= currentPosition) && 
				...
				if(configArray["streamscript"] == "lighttpd") {
					...
					streamObject.play(configArray["streamscript"]+"?file="+
