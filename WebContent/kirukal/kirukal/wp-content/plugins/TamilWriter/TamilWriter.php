<?php
/*
Plugin Name: Tamil Writer
Plugin URI: http://www.monusoft.com/Products/WordPressPlugin/tabid/187/Default.aspx
Description: This plugin lets the user write Tamil directly in post/page/comment box using transliterate Tamil keyboard layout.
Author: Monusoft
Version: 1.1
...
*/
if (!class_exists('TamilKeyPad')) {
class TamilKeyPad
{
    function TamilKeyPad()
    {
        add_action('comment_form', array(&$this, 'comment_form'));	//add Tamil Type Pad to comment form
        add_action('edit_form_advanced', array(&$this, 'post_form'));	//add Tamil Type Pad to Advanced Post
        add_action('simple_edit_form', array(&$this, 'post_form'));	//add Tamil Type Pad to Simple Post
        add_action('edit_page_form', array(&$this, 'post_form'));	//add Tamil Type Pad to Page
    }
    ...
    function render($parentElement,$txtareaName)
    {
        ...
        ?>
        <script language=javascript src="<?php echo get_settings('siteurl');?>/wp-content/plugins/TamilWriter/Parser.js"></script>
        <script language=javascript src="<?php echo get_settings('siteurl');?>/wp-content/plugins/TamilWriter/Tamil.js"></script>
		...
		<div style="display: left;align:left;" id="Tamil">
            <p style="align:left;">
            <label for="lang"><small>Select language:</small></label><input type="radio" value="Tamil" name="language" id="language1" style="width:auto;" onclick="isEng= false;" checked><label for="Tamil"><small>Tamil</small></label>&nbsp;<input type="radio" name="language" id="language2" style="width:auto;" value="English" onclick="isEng= true;"><label for="English"><small>English</small></label>
			<label for="ChangeLanguage"><small>(Press F12 to change language)</small></label>
         	<small class="commentmetadata"><a href="javascript:toggleLayer('TamilCharacterMap','i');" id="i">Show Keyboard</a></small>
            <div id="TamilCharacterMap" style="display:none;"><img src="<?php echo get_settings('siteurl');?>/wp-content/plugins/TamilWriter/TamilMap.JPG" alt="Tamil Character Map"/></div>
            </p>
        </div>
        <script language="JavaScript" type="text/javascript">
			var html, urlp;
            ...
            var sub = document.getElementById("Tamil");
