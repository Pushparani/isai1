...
function anarchy_mce_plugins($plugins) {    
	array_push($plugins, "-anarchyswf","-anarchydcr","-anarchymedia");    
	return $plugins;
}
...
// Required Quicktime version ='<?php echo $qtversion; ?>' - To set the minimum version go to Quicktime player section below and edit (quicktime.ver<?php echo $qtversion; ?>) on or around line 234.
...
Anarchy.MOV = {
	...
	toggle: function(img, url) {
		if (Anarchy.MOV.playimg == img) Anarchy.MOV.destroy()
			...
			if (quicktime.ver<?php echo $qtversion; ?>) {
...
function edInsertVideoSite()
{
	...
	if (videoURL)
	{
		...
		anarchy_settext('<a href="' + videoURL + '" title="Anarchy Media Player - Right click to download file"><em>Download:</em></a> ');		
		}
		else if(googlevid.test(videoURL)) {
		videoID1 = videoURL.replace(/\.(.*)\/videoplay/, '.google.com/googleplayer.swf')
		videoID = videoID1.replace(/&(.*)?/, "")
		anarchy_settext('<code>[kml_flashembed movie="' + videoID + '" width="<?php echo $googlewidth; ?>" height="<?php echo $googleheight; ?>" wmode="transparent" /]</code>');		
		}
		else if(youtubevid.test(videoURL)) {
		videoID1 = videoURL.replace(/watch\?v\=/, 'v/')
		videoID = videoID1.replace(/&(.*)?/, "")
		anarchy_settext('<code>[kml_flashembed movie="' + videoID + '" width="<?php echo $youtubewidth; ?>" height="<?php echo $youtubeheight; ?>" wmode="transparent" /]</code>');		
		}
		else if(dailymotionvid.test(videoURL)) {
		videoID = videoURL.replace(/^(.*)src=\"(.*)\" type=\"(.*)$/i, "$2")
		anarchy_settext('<code>[kml_flashembed movie="' + videoID + '" width="<?php echo $dailymotionwidth; ?>" height="<?php echo $dailymotionheight; ?>" wmode="transparent" /]</code>');		
		}
		else if(ifilmvid.test(videoURL)) {
		videoID1 = videoURL.replace(/http:\/\/www\.ifilm\.com\/video\/([a-z0-9])/i, "$1") 
		videoID = videoID1.replace(/\?(.*)?/, "")
		anarchy_settext('<code>[kml_flashembed movie="http://www.ifilm.com/efp" width="<?php echo $ifilmwidth; ?>" height="<?php echo $ifilmheight; ?>" fvars="flvbaseclip=' + videoID + '" wmode="transparent" /]</code>');	
		}
		else if(metacafevid.test(videoURL)) {
		videoID1 = videoURL.replace(/watch/i, "fplayer") 
		videoID = videoID1.replace(/\/$/, ".swf")
		anarchy_settext('<code>[kml_flashembed movie="' + videoID + '" width="<?php echo $metacafewidth; ?>" height="<?php echo $metacafeheight; ?>" wmode="transparent" /]</code>');	
		}
		else if(goearvid.test(videoURL)) {
		videoID1 = videoURL.replace(/http:\/\/www\.goear\.com\/listen\.php\?v=([a-z0-9])/i, "$1") 
		videoID = videoID1.replace(/\&(.*)?/, "")
		anarchy_settext('<code>[kml_flashembed movie="http://www.goear.com/files/localplayer.swf" width="366" height="75" fvars="file=' + videoID + '" wmode="transparent" /]</code>');	
		}
		...
		anarchy_settext('<code>[kml_flashembed movie="http://lads.myspace.com/videos/vplayer.swf" width="<?php echo $myspacewidth; ?>" height="<?php echo $myspaceheight; ?>" fvars="m=' + videoID + ';type=video" wmode="transparent" /]</code>');	
		}
		...
		anarchy_settext('<code>[kml_flashembed movie="http://flash.revver.com/player/1.0/player.swf" width="<?php echo $revverwidth; ?>" height="<?php echo $revverheight; ?>" fvars="mediaId=' + videoID + '" wmode="transparent" /]</code>');	
		}		
		else if(ipodplayer.test(videoURL)) {
		videoID = videoURL.replace(/^(.*)src=\"(.*)\" quality=\"(.*)$/i, "$2")
		anarchy_settext('<code>[kml_flashembed movie="' + videoID + '" width="300" height="330" wmode="transparent" /]</code>');		
		}
		else if(atomvid.test(videoURL)) {
		videoID = videoURL.replace(/http:\/\/www\.atomfilms\.com(:80)?\/film\/(.*)/i, "$2") 
		videoID = videoID.replace(/\?(.*)?/, "")
		videoID = videoID.replace(/\.jsp/, "")
		anarchy_settext('<code>[kml_flashembed movie="http://www.atomfilms.com:80/a/autoplayer/shareEmbed.swf" width="<?php echo $atomwidth; ?>" height="<?php echo $atomheight; ?>" fvars="keyword=' + videoID + '" /]</code>');	
		}
		else if(breakvid.test(videoURL)) {
		videoID = videoURL.replace(/^(.*)src=\"(.*)\" type=\"(.*)$/i, "$2")
		anarchy_settext('<code>[kml_flashembed movie="' + videoID + '" width="<?php echo $breakwidth; ?>" height="<?php echo $breakheight; ?>" wmode="transparent" /]</code>');		
		}
		else {
	...
	if (tag == 'swf')
	{
		...
		if (URL)
		{
			...
			if (width)
			{
				...
				if (height)
				{
					anarchy_settext('<code>[kml_flashembed movie="' + URL + '" width="' + width +'" height="' + height + '" wmode="transparent" /]</code>');
				}
			}
		}
	}
...
function kml_flashembed_build_fo_script($atts) {
	...
	$fvars				= (isset($fvars)) ? $fvars : array();				
									...
									$out[] = '';
						  	  		...
						  	  		$out[] = '	// <![CDATA[';
									$out[] = '';
						  	  		$out[] = '	var so_' . $rand . ' = new SWFObject('.$movie . $id . $width . $height . $fversion . $bgcolor . $quality . $xiredirecturl . $redirecturl . $detectKey . ');';
	...
	if ($useexpressinstall) {
		$xiswf = $_SERVER['REQUEST_URI'] . str_replace('\\', '/', dirname(preg_replace('/^.*wp-content[\\\\\/]plugins[\\\\\/]/', 'wp-content/plugins/', __FILE__))) . '/lib/expressinstall.swf';
