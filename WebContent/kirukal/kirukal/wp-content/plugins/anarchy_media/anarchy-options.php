...
// RTE buttons
	...
	update_option('anarchy_wysiwyg', $_POST['anarchy_wysiwyg']);
...
// AtomFilms
	if($_POST['anarchy_atomwidth'] == $atomwidthD || $_POST['anarchy_atomwidth'] == '' and $atomwidth == $atomwidthD )  : /* Do nothing */ ; 
	else : update_option('anarchy_atomwidth', $_POST['anarchy_atomwidth']); 
	endif;
	
	if($_POST['anarchy_atomheight'] == $atomheightD || $_POST['anarchy_atomheight'] == '' and $atomheight == $atomheightD )  : /* Do nothing */ ; 
	else : update_option('anarchy_atomheight', $_POST['anarchy_atomheight']); 
	endif;
...
<?php } ?>
		...
		<p>
			<label><input name="anarchy_wysiwyg" type="checkbox" id="anarchy_wysiwyg" value="true" 
<?php if($wysiwyg == 'true') {?>
				checked="checked" 
<?php } ?>
				/> <strong>Enable extended WYSIWYG</strong></label> (disabled by default as it can conflict with other wysiwyg plugins) 
		</p>
		<p class="submit">
...
<?php
          ...
          px
<!--************************ AtomFilms **************************-->
		<h3>AtomFilms</h3> <label>Width:</label> 
<?php 
            echo '<input name="anarchy_atomwidth" type="text" id="anarchy_atomwidth" style="width:40px;" value="'.$atomwidth.'"/>';
          ?>
		px&nbsp;&nbsp; <label>Height:</label> 
<?php
            echo '<input name="anarchy_atomheight" type="text" id="anarchy_atomheight" style="width:40px;" value="'.$atomheight.'"/>';
          ?>
          px
<!--************************ Break **************************-->
