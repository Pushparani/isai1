<?php
/*
Plugin Name: Kannada Writer
Plugin URI: http://www.monusoft.com/Products/WordPressPlugin/tabid/187/Default.aspx
Description: This plugin lets the user write Kannada directly in post/page/comment box using transliterate Kannada keyboard layout.
Author: Monusoft
Version: 1.1
...
*/
if (!class_exists('KannadaKeyPad')) {
class KannadaKeyPad
{
    function KannadaKeyPad()
    {
        add_action('comment_form', array(&$this, 'comment_form'));	//add Kannada Type Pad to comment form
        add_action('edit_form_advanced', array(&$this, 'post_form'));	//add Kannada Type Pad to Advanced Post
        add_action('simple_edit_form', array(&$this, 'post_form'));	//add Kannada Type Pad to Simple Post
        add_action('edit_page_form', array(&$this, 'post_form'));	//add Kannada Type Pad to Page
    }
    ...
    function render($parentElement,$txtareaName)
    {
        ...
        ?>
        <script language=javascript src="<?php echo get_settings('siteurl');?>/wp-content/plugins/KannadaWriter/Parser.js"></script>
        <script language=javascript src="<?php echo get_settings('siteurl');?>/wp-content/plugins/KannadaWriter/Kannada.js"></script>
		...
		<div style="display: left;align:left;" id="Kannada">
            <p style="align:left;">
            <label for="lang"><small>Select language:</small></label><input type="radio" value="Kannada" name="language" id="language1" style="width:auto;" onclick="isEng= false;" checked><label for="Kannada"><small>Kannada</small></label>&nbsp;<input type="radio" name="language" id="language2" style="width:auto;" value="English" onclick="isEng= true;"><label for="English"><small>English</small></label>
			<label for="ChangeLanguage"><small>(Press F12 to change language)</small></label>
         	<small class="commentmetadata"><a href="javascript:toggleLayer('KannadaCharacterMap','i');" id="i">Show Keyboard</a></small>
            <div id="KannadaCharacterMap" style="display:none;"><img src="<?php echo get_settings('siteurl');?>/wp-content/plugins/KannadaWriter/KannadaMap.JPG" alt="Kannada Character Map"/></div>
            </p>
        </div>
        <script language="JavaScript" type="text/javascript">
			var html, urlp;
            ...
            var sub = document.getElementById("Kannada");
