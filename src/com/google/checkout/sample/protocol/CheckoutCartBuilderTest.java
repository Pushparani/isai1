...
public class CheckoutCartBuilderTest extends TestCase {
  ...
  public void testCreateShoppingCart() throws Exception {
    ...
    Document shopCart = pBuilder.createCheckoutCart(cart, makeMerchantCheckoutFlow());
  ...
  private MerchantCheckoutFlowSupport makeMerchantCheckoutFlow() 
      throws Exception {
    CheckoutCartBuilder pBuilder = CheckoutCartBuilder.getInstance();
    ...
    stateAreasList.add(nyState);
     ...
     USStateArea state = pBuilder.createStateArea("ca");
     DefaultTaxRule defaultTaxRule =pBuilder.createDefaultTaxRule(state,
                0.07F, true);
     ruleList.add(defaultTaxRule);

     DefaultTaxTable defaultTaxTable =pBuilder.createDefaultTaxTable(ruleList);
     MerchantCalculations  defaultMerchantCalculations;

     String editCartUrl="http://207.138.176.41/basket/basketmain.jsp";
     String continueShoppingUrl="http://207.138.176.41/";
     String merchantCalculateUrl="http://207.138.176.41/GoogleCallback";


     defaultMerchantCalculations =pBuilder.createMerchantCalculations(true,true,  merchantCalculateUrl);
     return pBuilder.createMerchantSupport(null, defaultTaxTable, null,
                          defaultMerchantCalculations, editCartUrl,continueShoppingUrl);    
  }
